...
export default function UpdatePassword() {
  ...
  async function newpassApi(data) {
    await axios.put(`${BaseUrl}/api/v1/auth/resetPassword`, data).then((req) => {
     if(req.data.token){
        navigate('/login')
     }
    ...
    <>
      ...
      <form onSubmit={NewPassForm.handleSubmit} className="w-6/12 mx-auto ">
        ...
        <button disabled={!(NewPassForm.isValid && NewPassForm.dirty)}
          type="submit" className="text-white bg-active hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800  disabled:bg-active disabled:bg-opacity-40">Submit</button>
      </form>
